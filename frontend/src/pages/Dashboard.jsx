...
const Dashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState('summary'); // 'summary' or 'verification'
    ...
    <div className="min-h-[calc(100vh-4rem)] bg-ash-grey pb-12 pt-6 px-4 sm:px-8">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex items-center gap-3">
            <div className="flex bg-gray-100 p-1 rounded-xl mr-2">
              <button 
                ...
                className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${activeTab === 'summary' ? 'bg-white text-emerald shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
              >
                Statistik
              </button>
              <button 
                onClick={() => setActiveTab('verification')}
                className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${activeTab === 'verification' ? 'bg-white text-emerald shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
              >
                Verifikasi
              </button>
            </div>
        ...
        {activeTab === 'summary' ? (
          ...
          >
            {data.rows.filter(row => row[8]).map((row, idx) => {
              const nama = row[2];
              const nominal = row[7];
              const buktiUrl = row[8];
              const keterangan = row[9];
              const thumb = getDriveThumbnail(buktiUrl);
            ...
            {data.rows.filter(row => row[8]).length === 0 && (
              <div className="col-span-full p-12 bg-white rounded-3xl border-2 border-dashed border-gray-100 text-center text-gray-400">
                Belum ada data bukti pembayaran untuk diverifikasi.
              </div>
            )}
          </motion.div>
        )}
